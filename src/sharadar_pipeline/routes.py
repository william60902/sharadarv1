...
_ROUTES = {
    Deployment.DEV: SharadarRoute(
        ...
        artifact_root=Path("/Volumes/Pentagon_Quant/sharadar/dev"),
...
def require_route_io(
    ...
    *,
    ...
    if write and route.deployment is Deployment.PROD:
        raise ProductionRouteLocked("SHARADAR_PROD is routed but not write-authorized")
    if write and not route.live_write_authorized:
